...
// Globe height settling is driven independently from render callbacks. A
// precise 60-ish Hz timer prevents pointer-event frequency, tile arrivals,
// or QRhi frame coalescing from becoming the animation cadence.
...
{
    if (!(time_constant_seconds > 0.0) || !(elapsed_seconds > 0.0))
        return current;
...
// relative to the camera's current globeRenderOriginEcef(), i.e. taken
// directly from MapGlobeNetworkScene's own vertex/instance data (which
    ...
    int viewport_width, int viewport_height, bool ndc_y_up)
{
    ...
    const double ndc_x = double(clip.x() / clip.w());
      ...
      surface_name(surface_name)
{
    ...
    this->globe_terrain_follow_timer->setTimerType(Qt::PreciseTimer);
    this->globe_terrain_follow_timer->setInterval(
        GlobeTerrainFollowTimerIntervalMs);
    connect(this->globe_terrain_follow_timer, &QTimer::timeout, this, [this]
    {
    ...
    connect(this->map_model, &MapModel::viewModeChanged, this, [this](MapViewMode view_mode)
    {
        ...
        else
        {
            if (this->globe_terrain_follow_timer != nullptr)
                this->globe_terrain_follow_timer->stop();
            this->globe_terrain_follow_clock.invalidate();
            this->map_model->setViewGlobeCameraCollisionLiftM(0.0);
        }
        syncViewState();
            ...
            this, [this](double exaggeration)
    {
        ...
        if (globe_changed)
        {
            this->globe_network_backend.invalidateGeometry();
            this->globe_network_backend.invalidateHighlights();
            this->globe_network_backend.invalidateFlowDirections();
            this->globe_network_backend.invalidateIcons();
            ...
            this, [this](double offset_m)
    {
        ...
        this->tank_upload_pending = true;
        this->reservoir_upload_pending = true;
        this->globe_network_backend.invalidateAll();
        update();
    });
    ...
    {
        // Screen-ray generation now lives on MapGlobeCamera, so keep its
        // canonical state synchronized immediately with input-driven camera
        // changes rather than waiting for the next render() call.
        syncViewState();
    ...
    {
        // Vertical offset/collision lift are part of the canonical Globe
        // camera state used by screenRay(). These settle independently of the
        // general viewGlobeCameraChanged signal.
        syncViewState();

        // Terrain settling is deliberately not a general Globe-camera event:
        // it must repaint this surface without driving footer controls or
        // editor/monitor movement synchronization on every animation tick.
        update();
    });
...
MapRhiHit MapRhiWidget::globeHitTest(const QPointF &screen_position) const
{
    ...
    // entity set (see setHiddenEntityUuids()), so reusing this->scene here
    // for uuid/hidden-state lookups is safe and avoids adding a redundant
    // accessor to MapGlobeNetworkScene purely for that. Every position
    // read out of globe_network_scene below, in contrast, is Globe-specific:
    // already relative to the camera's current render origin (see
    // MapGlobeNetworkScene::ecefPosition()), matching exactly what
    ...
    // own hit path. Use the same backend-independent Globe screen ray as DEM
    // picking and intersect it with the exact CPU-side triangles uploaded for
    // the rendered models. Model vertices are relative to globeRenderOriginEcef(),
    // so translate the absolute ECEF camera origin into that same local frame.
    MapGlobeScreenRay globe_mesh_ray;
    const bool have_globe_mesh_ray = this->camera.globeScreenRay(
        screen_position, &globe_mesh_ray);
    QVector3D globe_mesh_ray_origin;
    if (have_globe_mesh_ray)
    {
        const GeoWgs84Ellipsoid::EcefPositionD render_origin_ecef =
            this->camera.globeRenderOriginEcef();
        globe_mesh_ray_origin = QVector3D(
            float(globe_mesh_ray.origin_ecef.x - render_origin_ecef.x),
            float(globe_mesh_ray.origin_ecef.y - render_origin_ecef.y),
            float(globe_mesh_ray.origin_ecef.z - render_origin_ecef.z));
    }

    quint32 best_tank_render_id = 0;
    double best_tank_distance = std::numeric_limits<double>::infinity();
    if (have_globe_mesh_ray && !this->tank_model_vertices.isEmpty())
    {
        for (qsizetype vertex_index = 0;
             ...
             vertex_index += 3)
        {
            ...
            if (!mapGlobeRayTriangleIntersectionDistance(
                    globe_mesh_ray_origin, globe_mesh_ray.direction,
                    a, b, c, &hit_distance)
    ...
    double best_reservoir_distance = std::numeric_limits<double>::infinity();
    if (have_globe_mesh_ray && !this->reservoir_model_vertices.isEmpty())
    {
        for (qsizetype vertex_index = 0;
    ...
    for (qsizetype vertex_index = 0; vertex_index + 5 < node_vertices.size();
         vertex_index += 6)
    {
        ...
        best_node_distance = distance;
        best_node_render_id = node_vertex.render_id;
        best_node_entity_type = node_vertex.entity_type;
    }

    if (best_node_render_id != 0)
    {
        for (const NetworkRenderNode &node : snapshot.nodes)
        {
            if (node.render_id != best_node_render_id
                || node.entity_type != best_node_entity_type
                || this->scene.isEntityHidden(node.uuid))
            {
                continue;
            }
    ...
    // Links. Each link segment contributes 6 vertices (two triangles) with
    // identical start_*/end_* fields per segment -- see
    // MapGlobeNetworkScene::appendLinkSegment() -- so reading them off
    // the first vertex of every group of 6 is enough, exactly as
    ...
    for (qsizetype vertex_index = 0; vertex_index + 5 < link_vertices.size();
         vertex_index += 6)
    {
        const MapRhiScene::LinkVertex &link_vertex = link_vertices.at(vertex_index);
        ...
        best_link_screen_distance = distance;
        best_globe_link_render_id = link_vertex.render_id;
        best_globe_link_entity_type = link_vertex.entity_type;
    }

    if (best_globe_link_render_id != 0)
    {
        for (const NetworkRenderLink &link : snapshot.links)
        {
            if (link.render_id != best_globe_link_render_id
                || link.entity_type != best_globe_link_entity_type
                || this->scene.isEntityHidden(link.uuid))
            {
                continue;
            }
...
void MapRhiWidget::setTerrainRepository(MapTerrainRepository *terrain_repository)
{
    ...
    // Globe mode. Initialise the under-camera height now and give that exact
...
void MapRhiWidget::renderGlobe(QRhiCommandBuffer *command_buffer, QRhiRenderTarget *target)
{
    ...
    // Must happen before the neutral MapGlobeRenderFrame is built and before
    // the concrete Globe QRhi backend below -- everything downstream in this
    // function that cares about the
    ...
    const QSize output_size = target->pixelSize();
    const MapGlobeRenderFrame globe_frame = mapGlobeBuildRenderFrame(
        this->camera.globeCamera(), this->globe_network_scene,
        this->applied_symbology, this->viewport_size, output_size);
    if (!globe_frame.isValid())
    {
        reportFailure(QStringLiteral("Invalid Globe render frame"));
        return;
    }
    ...
    QString globe_backend_failure;
    if (!this->globe_network_backend.ensureBuffers(
            *this->active_rhi, globe_frame.resources, &globe_backend_failure))
    {
        reportFailure(globe_backend_failure);
        return;
    }

    // Terrain receives the same backend-corrected matrix as the network
    // backend. The neutral frame remains in canonical clip space until this
    // QRhi boundary.
    QMatrix4x4 view_projection = this->active_rhi->clipSpaceCorrMatrix();
    view_projection *= globe_frame.canonical_view_projection;

    QRhiResourceUpdateBatch *resource_updates = this->active_rhi->nextResourceUpdateBatch();
    this->globe_network_backend.updateCameraUniforms(
        resource_updates, this->uniform_buffer.get(), *this->active_rhi,
        globe_frame, this->network_style_texture_size);
    uploadNetworkStyleTable(resource_updates);
    ...
    this->globe_network_backend.uploadGeometry(
        resource_updates, globe_frame.resources);
    ...
    if (!this->globe_renderer->prepare(
            resource_updates, view_projection, globe_frame.logical_viewport_size,
    ...
    // Heatmap texture bakes own separate update batches and offscreen passes.
    // Record them before the visible pass so its existing resource_updates
    // batch remains attached to the swapchain target exactly as before. A
    // failed GPU bake disables itself and is regenerated by the CPU fallback
    // on the immediately requested next frame.
    if (!this->globe_renderer->runPendingHeatmapGpuBakes(command_buffer))
    {
        resource_updates->release();
        update();
        return;
    }
    this->globe_renderer->runDiagnosticHeatmapGpuBake(command_buffer);
    ...
    MapRhiGlobeNetworkDrawResources globe_draw_resources;
    globe_draw_resources.junction_mesh_vertex_buffer =
        this->junction_mesh_vertex_buffer.get();
    globe_draw_resources.tank_vertex_buffer = this->tank_vertex_buffer.get();
    globe_draw_resources.reservoir_vertex_buffer = this->reservoir_vertex_buffer.get();
    globe_draw_resources.junction_shader_resource_bindings =
        this->junction_shader_resource_bindings.get();
    globe_draw_resources.icon_shader_resource_bindings =
        this->icon_shader_resource_bindings.get();
    globe_draw_resources.tank_shader_resource_bindings =
        this->tank_shader_resource_bindings.get();
    globe_draw_resources.reservoir_shader_resource_bindings =
        this->reservoir_shader_resource_bindings.get();
    globe_draw_resources.link_pipeline = this->link_pipeline.get();
    globe_draw_resources.flow_direction_pipeline =
        this->globe_flow_direction_pipeline.get();
    globe_draw_resources.selected_link_pipeline = this->selected_link_pipeline.get();
    globe_draw_resources.node_pipeline = this->node_pipeline.get();
    globe_draw_resources.node_overlay_pipeline = this->node_overlay_pipeline.get();
    globe_draw_resources.icon_overlay_pipeline = this->icon_overlay_pipeline.get();
    globe_draw_resources.tank_pipeline = this->tank_pipeline.get();
    globe_draw_resources.reservoir_pipeline = this->reservoir_pipeline.get();
    globe_draw_resources.junction_pipeline = this->globe_junction_pipeline.get();
    globe_draw_resources.junction_no_depth_pipeline =
        this->globe_junction_no_depth_pipeline.get();
    globe_draw_resources.link_xray_pipeline = this->link_xray_pipeline.get();
...
bool MapRhiWidget::createPipelines()
{
    ...
    if (!this->globe_junction_pipeline)
    {
        ...
        this->globe_junction_pipeline->setDepthTest(true);
    ...
    if (!this->junction_xray_pipeline || !this->globe_junction_no_depth_pipeline)
    {
        ...
        if (!this->globe_junction_no_depth_pipeline)
        {
            ...
            this->globe_junction_no_depth_pipeline->setShaderStages({
                {QRhiShaderStage::Vertex, vertex_shader},
                {QRhiShaderStage::Fragment, normal_fragment_shader}
            });
            ...
            this->globe_junction_no_depth_pipeline->setTopology(QRhiGraphicsPipeline::Triangles);
            this->globe_junction_no_depth_pipeline->setDepthTest(false);
            this->globe_junction_no_depth_pipeline->setDepthWrite(false);
            this->globe_junction_no_depth_pipeline->setCullMode(QRhiGraphicsPipeline::None);
            QRhiGraphicsPipeline::TargetBlend globe_solid_blend;
            globe_solid_blend.enable = true;
            this->globe_junction_no_depth_pipeline->setTargetBlends({globe_solid_blend});
            if (!this->globe_junction_no_depth_pipeline->create())
            {
                reportFailure(
                    QStringLiteral("Failed to create RHI globe no-depth junction pipeline"));
                return false;
            }
        }
    }
...
void MapRhiWidget::resetGpuResources()
{
    ...
    this->globe_network_backend.reset();
...
bool MapRhiWidget::globeCameraTerrainElevationAtCoordinate(
    const CoordinateWGS84 &coordinate, double *elevation_m,
    bool request_missing_tile) const
{
    ...
    {
        // Globe relief is deliberately disabled below this LOD, where the
        // rendered surface is the zero-height ellipsoid.
        *elevation_m = 0.0;
        return true;
    }
    ...
    const int target_terrain_zoom = qBound(
        CameraTerrainMinimumZoom, imagery_zoom, configured_maximum_zoom);
    const QString dataset = cameraTerrainDatasetId();

    // Request only the exact current LOD. While it is loading, a cached
    // coarser tile gives the spring a stable provisional surface instead of
    // snapping the rig to zero and back as tile boundaries are crossed.
    for (int terrain_zoom = target_terrain_zoom;
         terrain_zoom >= CameraTerrainMinimumZoom; --terrain_zoom)
    {
        ...
        if (terrain_tile == nullptr)
        {
            if (request_missing_tile && terrain_zoom == target_terrain_zoom)
            {
        ...
        if (terrain_tile->vertical_datum != MapTerrainVerticalDatum::Wgs84Ellipsoid
            && terrain_tile->vertical_datum != MapTerrainVerticalDatum::Egm96
            && terrain_tile->vertical_datum != MapTerrainVerticalDatum::Egm2008)
        {
            // Match MapRhiGlobeRenderer: unknown/local heights cannot be
            // interpreted as displacement from the global ellipsoid.
            continue;
        }
...
void MapRhiWidget::syncGlobeTerrainAwareCameraHeight(bool request_missing_tile)
{
    if (this->globe_terrain_camera_sync_active)
        return;
    ...
    QScopedValueRollback<bool> sync_guard(
        this->globe_terrain_camera_sync_active, true);
    ...
    const double distance_m = qMax(
        MapModel::MinViewGlobeDistanceM,
        this->map_model->viewGlobeDistanceM());
    const double current_offset_m =
        this->map_model->viewGlobeVerticalOffsetM();
    const double current_collision_lift_m =
        this->map_model->viewGlobeCameraCollisionLiftM();
    ...
    // perpendicularly beneath the camera; centerLon()/centerLat() is the
    // crosshair/orbit target and can be hundreds of meters away at low pitch.
    const GeoWgs84Ellipsoid::EcefPositionD current_eye =
        GeoWgs84Ellipsoid::orbitCameraEyeEcefD(
            this->map_model->centerLon(), this->map_model->centerLat(),
            this->map_model->viewGlobeYawDeg(), pitch_deg, distance_m,
            current_offset_m, current_collision_lift_m);
    CoordinateWGS84 camera_coordinate;
    if (!GeoWgs84Ellipsoid::ecefToGeodetic(
            current_eye, &camera_coordinate.longitude_deg,
            &camera_coordinate.latitude_deg))
    {
        return;
    }

    double camera_terrain_elevation_m = 0.0;
    const bool terrain_available = this->terrain_repository != nullptr
        && globeCameraTerrainElevationAtCoordinate(
            camera_coordinate, &camera_terrain_elevation_m,
            request_missing_tile);
    if (!terrain_available)
    {
        // Match the surface actually rendered while no cached DEM level is
        // available. The monotonic filter approaches this zero-height fallback
        // instead of snapping to it; a later DEM arrival is filtered likewise.
        camera_terrain_elevation_m = 0.0;
    }

    const MapGlobeVerticalTransform vertical_transform(
        this->map_model->view3dVerticalExaggeration(),
        this->map_model->view3dNetworkGroundOffsetM());
    const double rendered_camera_terrain_elevation_m =
        vertical_transform.terrainHeightM(camera_terrain_elevation_m);

    // Compute the height naturally provided by yaw/pitch/distance over the
    // bare ellipsoid. Usually it is far above the minimum; retaining this
    // exact curvature-aware calculation also makes low-pitch close views
    // safe without treating distance*sin(pitch) as exact Globe altitude.
    const GeoWgs84Ellipsoid::EcefPositionD zero_height_eye =
        GeoWgs84Ellipsoid::orbitCameraEyeEcefD(
            this->map_model->centerLon(), this->map_model->centerLat(),
            this->map_model->viewGlobeYawDeg(), pitch_deg, distance_m);
    double nominal_lon_deg = 0.0;
    double nominal_lat_deg = 0.0;
    double nominal_eye_height_m = 0.0;
    if (!GeoWgs84Ellipsoid::ecefToGeodetic(
            zero_height_eye, &nominal_lon_deg, &nominal_lat_deg,
            &nominal_eye_height_m))
    {
        return;
    }
    // This is deliberately the one exact DEM sample perpendicularly beneath
    // the eye. The previous footprint combined independently cached LODs and
    // changed membership at tile boundaries, creating discontinuous targets.
    const double target_offset_m =
        rendered_camera_terrain_elevation_m + qMax(
            0.0,
            MapModel::MinViewGlobeCameraGroundClearanceM
                - nominal_eye_height_m);

    double elapsed_seconds = 1.0 / 60.0;
    if (this->globe_terrain_follow_clock.isValid())
    {
        elapsed_seconds = qBound(
            0.0,
            double(this->globe_terrain_follow_clock.nsecsElapsed()) * 1e-9,
            GlobeTerrainMaximumTimeStepSeconds);
        this->globe_terrain_follow_clock.restart();
    }
    else
    {
        this->globe_terrain_follow_clock.start();
    }

    double next_offset_m = current_offset_m;
    double next_collision_lift_m = current_collision_lift_m;
    double required_collision_lift_m = 0.0;

    if (orbit_target_frozen)
    {
        // Keep the captured orbit target fixed. Only the eye-only clearance
        // target changes while rotating, and that path is filtered below by
        // the same monotonic mechanism as ordinary terrain following.
        const GeoWgs84Ellipsoid::EcefPositionD unlifted_eye =
            GeoWgs84Ellipsoid::orbitCameraEyeEcefD(
                this->map_model->centerLon(), this->map_model->centerLat(),
                this->map_model->viewGlobeYawDeg(), pitch_deg, distance_m,
                next_offset_m);
        double unlifted_lon_deg = 0.0;
        double unlifted_lat_deg = 0.0;
        double unlifted_eye_height_m = 0.0;
        if (!GeoWgs84Ellipsoid::ecefToGeodetic(
                unlifted_eye, &unlifted_lon_deg, &unlifted_lat_deg,
                &unlifted_eye_height_m))
        {
            return;
        }
        required_collision_lift_m = qMax(
            0.0,
            rendered_camera_terrain_elevation_m
                + MapModel::MinViewGlobeCameraGroundClearanceM
                - unlifted_eye_height_m);
    }

    // The eye-only lift is needed only while rotating around a frozen pivot.
    // On return to Pan, ease it to zero and transfer exactly the released
    // amount into the whole-rig offset. That keeps the eye position continuous
    // while the target itself also returns smoothly to ordinary terrain follow.
    const double collision_lift_before_step_m = next_collision_lift_m;
    next_collision_lift_m = approachExponentially(
    ...
    if (!orbit_target_frozen)
    {
        next_offset_m += collision_lift_before_step_m - next_collision_lift_m;
        next_offset_m = approachExponentially(
    ...
    if (follow_animating || collision_animating)
    {
        if (this->globe_terrain_follow_timer != nullptr
            && !this->globe_terrain_follow_timer->isActive())
        {
            this->globe_terrain_follow_timer->start();
        }
        update();
    }
    else
...
void MapRhiWidget::syncGlobeHeatmapOverlay(double solid_fraction)
{
    ...
    // class comment and MapGlobeNetworkScene's for why it needs a
